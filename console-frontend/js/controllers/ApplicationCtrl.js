...
angular.module('appControllers').controller('ApplicationCtrl', ['$scope', '$filter', '$cookies',
  ...
  'MetricService', 'UtilService', 'ConfigService', function($scope, $filter, $cookies, DeploymentManagerService,
    ...
    $scope.sparkLogs = "Logs from appliction controller";    /* XXX */
    ...
    $scope.startOrStopApplication = function(name, status) {
      ...
      if (action !== undefined) {
        displayConfirmation("Are you sure you want to " + action + " " + name + "?", function() {
          ...
          $scope.getAppState(name);
          $scope.getAppLogs(name);
    ...
    $scope.createNewApp = function() {
     ...
     /*XXX */
    $scope.appLogData = '';
    $scope.getAppLogs = function(appName) {
      $interval(function(){
        DeploymentManagerService.getApplicationLogs(appName).then(function(data) {
          $scope.appLogData = data;
        });
      }, 4000);
    };

    $scope.appStateData = 'Created';
    ...
    $scope.getAppLogs2 = function() {
      ...
      //  $scope.getLog4 = data;
        $scope.getLog4 = DeploymentManagerService.getApplicationTestLogs();
